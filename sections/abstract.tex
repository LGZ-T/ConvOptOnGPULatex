...
Our approach works for convolution operations performed on the width and height dimensions. When sliding a filter on the width dimension,
we shuffle instructions to exchange overlapped columns of input to \FIXME{reduce/improve???}. When sliding a filter on the height
dimension, we multiply each overlapped row of input with multiple rows of a filter to \FIXME{reduce/improve????}.
