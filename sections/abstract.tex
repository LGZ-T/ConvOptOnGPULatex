...
width and the height dimensions. For convolution computations on the width dimension, we exploit shuffle instructions to exchange the
...
%
We apply our approach to {\color{red}single-channel 2D and depth-wise convolutions and evaluate them on an NVIDIA RTX 2080Ti GPU}. For the single-channel 2D
convolution, our approach delivers over 2x faster performance than the state-of-the-art image processing libraries. {\color{red}For the depth-wise convolution,
we obtain up to 4x speedups over the fastest algorithm of cuDNN.}
