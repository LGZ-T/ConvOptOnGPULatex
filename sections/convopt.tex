...
this way, each GPU thread calculates one column of the output elements. This is done through a number of steps. {\color{red}First, each thread block loads the filter into shared memory and divide the filter into a combination of 3-column and 5-column sub-filters. Second, each thread
calculates the address of the first input element it needs (Lines 9). Next, for each output element and sub-filter, each thread loads corresponding input elements into $iTemp$ and passes it to Algorithms \ref{algo:basic} and
\ref{algo:basic2} to fulfill the row vector $iTemp$ (Line 10). Then, each thread passes the fulfilled vector $iTemp$ to Algorithm \ref{algo:rowreuse}
to calculate multiple output elements and store results in register array $sum$ (Line 11). Finally, when calculation of one output element is completed, we write the corresponding result in $sum$ into the result array $O$ (Line 13).} 