...
convolution) and can not fully utilize DP4A instruction to accelerate INT8 convolution. \RV{We note taht PyTorch and TensorFlow do not
...
of the GPU. A special parameter tuple (we take parameter tuples generated for 2080Ti as examples) is the layer configuration \textbf{CONV9}
with $I_N = 1$. The width of the logical layout of \textbf{CONV9} is small. Hence we would like to search for a small $C_{num}$. However,
...
obtain 8.5\% and 11.7\% improvements on average over cuDNN PRECOMP on 2080Ti and Xavier, respectively. \RV{For EfficientNet-B0 with FP32,
our approach improves cuDNN IMPLICIT by 14.4\% and 12.3\% on average on 2080Ti and Xavier, respectively. For EfficientNet-B0 with INT8, we
obtain 9.9\% and 9.6\% improvements on average over cuDNN PRECOMP on 2080Ti and Xavier, respectively.} Table \ref{tab:traintime} shows that
our approach reduces the training time by 8.5\% on average compared to cuDNN IMPLICIT on 2080Ti. The results show that our approach can
