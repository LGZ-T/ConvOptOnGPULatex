...
\subsubsection{Overall results}
\mypara{FP32 implementation.} Since that cuDNN is close-sourced, further analysis into the root casue is difficult. We hypothesize that
...
\mypara{INT8 implementation.} We found using FP32 gives over a speedup of more than $10\times$ over the INT8 version for depthwise
...
Nonetheless, our approach gives a \FIXME{xx} speedup when using INT8 over CuDNN.
...
\begin{figure}[t!]
    ...
    \qquad
    %\vspace{5mm}
    \subfloat[][Ratios of executed LDG (load from global memory) instruction counts of cuDNN implicit algorithm to our approach (\FIXME{HOW did you compute the ratio????}.]{\includegraphics[width=0.9\columnwidth]{./figure/ldginst.eps}\label{fig:dwldginst}}
    \vspace{-2mm}
