...
\subsection{Competing Methods} We compare our approach against the state-of-the-art convolution library cuDNN which supports a wide range of convolution operations,
 including depthwise and pointwise convolutions on GPU.  Moreover, cuDNN can execute GEMM-, FFT- and Winograd-based convolutions.
...
We apply our approach to depthwise convolution and pointwise convolution.  We run each test case ten times with batch sizes 1, 8, 16, 32,
64 and 128 on an unloaded machine and report the averaged running time. We found little variance during execution runs, less than 2\%. We
run convolutions with two data types, 32-bit floating points (FP32) for normal CNNs and 8-bit integer (INT8) for quantized CNNs
...
H, W$ respectively denote for the bath size, the number of channels, the height and the width. \FIXME{Why using two different formats????}
...
\begin{figure*}[!t]
\centering
\subfloat[Speedups on 2080Ti for the $3 \times 3$ fitler.]{\includegraphics[width=0.4\textwidth,height=5.5cm]{./figure/dwspeeduprtxf3.eps}
	\label{fig:dwspeeduprtxf3}}
\hspace{0em}
\subfloat[Speedups on 2080Ti for the $5 \times 5$ fitler.]{\includegraphics[width=0.4\textwidth,height=5.5cm]{./figure/dwspeeduprtxf5.eps}
	\label{fig:dwspeeduprtxf5}}
\hspace{0em}
\subfloat[Speedups on Xavier for the $3 \times 3$ fitler.]{\includegraphics[width=0.4\textwidth,height=5.5cm]{./figure/dwspeedupjetsonf3.eps}
	\label{fig:dwspeedupjetsonf3}}
\hspace{0em}
\subfloat[Speedups on Xavier for the $5 \times 5$ fitler.]{\includegraphics[width=0.4\textwidth,height=5.5cm]{./figure/dwspeedupjetsonf5.eps}
...
Fig. \ref{fig:dwratio} reports the measured LDG (load from global memory) instruction counts and SM utilization for the fast cuDNN implicit algorithm and our approach when using a $3 \times 3$ filter and a batch size of 32 on 2080Ti. Other configurations have a similar performance.
We can see in Fig. \ref{fig:dwsmutil} that cuDNN implicit algorithm has an average of $2\times$ higher SM utilization compared to our approach.
...
Different from cuDNN implicit, we focus on optimizing memory performance for depthwise convolution. We use row and column reuse techniques to reduce memory operations by a maximum of $4.5\times$ lower executed LDG instructions, as shown in Fig. \ref{fig:dwldginst}.
...
\captionsetup[subfloat]{labelformat=empty,skip=0pt}
	\centering
	\subfloat[]{\includegraphics[width=\textwidth,height=6.2cm]{./figure/pwspeeduprtx.eps}
	\label{fig:pwspeeduprtx}}
\vspace{-5mm}
	\subfloat[]{\includegraphics[width=\textwidth,height=6.2cm]{./figure/pwspeedupjetson.eps}
...
\captionsetup[subfloat]{labelformat=empty,skip=0pt}
	...
	\centering
	\subfloat[]{\includegraphics[width=\textwidth,height=6.2cm]{./figure/pwspeeduprtxint8.eps}
	\label{fig:pwspeeduprtxint8}}
	\vspace{-5mm}
	\subfloat[]{\includegraphics[width=\textwidth,height=6.2cm]{./figure/pwspeedupjetsonint8.eps}
