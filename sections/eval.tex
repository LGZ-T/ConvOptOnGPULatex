...
\caption{Speedups of \RV{cuDNN IMPLICIT, our approach, Tensorflow and PyTorch} over the baseline implementation for FP32 depthwise convolution with filters of size $3 \times 3$ and $5 \times 5$ on two platforms.} \label{fig:dwspeedup}
\end{figure*}



\subsubsection{Setup} In this experiment, we compare our approach against the depthwise convolution implementations of cuDNN, PyTorch and TensorFlow.
During the experiments, we have compared our approach to seven algorithms in cuDNN, including IMPLICIT\_GEMM (IMPLICIT), IMPLICIT\_PRECOMP\_GEMM (PRECOMP), GEMM, FFT, FFT\_TILING (TILING), WINOGRAD and WINOGRAD\_NONFUSED (NONFUSED). We found IMPLICIT gives the best performance \RV{in all our test cases. Therefore, we report the results by comparing our approach against the IMPLICIT algorithm of cuDNN and take cuDNN GEMM as the baseline in this evaluation.} Table \ref{tab:depconvconfigs} gives the layer configurations used in this experiment where the notations were defined earlier in Section \ref{sec:roadmap}.


\subsubsection{Overall results}
\mypara{FP32 implementation.} Fig. \ref{fig:dwspeedup} shows that our approach gives the best speedup in nearly all test cases. Table \ref{tab:dwspeedups} presents average speedups of cuDNN IMPLICIT, our approach, TensorFlow and PyTorch over cuDNN GEMM for $3 \times 3$ and $5 \times 5$ filter sizes on 2080Ti and Xavier.
...
TensorFlow achieves better speedups than PyTorch because it employs several specially designed kernels to increase GPU utilization for different input sizes. However, both TensorFlow and PyTorch do not optimize memory performance for depthwise convolutions. Compared to TensorFlow, our approach achieves an average speedup of $1.5 \times$ and $1.6 \times$ when using a $3 \times 3$ filter on 2080Ti and Xavier respectively, and $2.2 \times$ and $1.7 \times$ when using a $5 \times 5$ filter on 2080Ti and Xavier respectively. Since cuDNN is closed source, we analyze its performance through CUDA Nsight Compute \cite{nsightcompute} and present the results in Section \ref{sec:dwfurther}. Overall, our approach improves cuDNN IMPLICIT by $2.0\times$ and $1.4\times$ when using a $3 \times 3$  filter on 2080Ti and Xavier respectively, and $3.5\times$ and $2.1\times$ when using a $5 \times 5$ filter on 2080Ti and Xavier respectively.
%=======
%IMPLICIT\_PRECOMP\_GEMM (PRECOMP), GEMM, FFT, FFT\_TILING (TILING), WINOGRAD and WINOGRAD\_NONFUSED (NONFUSED). We found IMPLICIT gives the
%best performance \RV{in all our test cases. Therefore, we normalize the performance against the IMPLICIT algorithm of cuDNN in this
%evaluation.} Table \ref{tab:depconvconfigs} gives the layer configurations used in this experiment where the notations were defined earlier
%in Section \ref{sec:roadmap}.
%
%
%\subsubsection{Overall results}
...
%approach delivers an average speedup of $2.2\times$ and $42.8\times$ for the $3 \times 3$ filter on 2080Ti and Xavier respectively, and
%$3.9\times$ and $39.4\times$ for the $5 \times 5$ filter on 2080Ti and Xavier respectively. The cuDNN IMPLICIT algorithm achieves an
%average speedup of $1.1\times$ and $32.8\times$ for the $3 \times 3$ filter on 2080Ti and Xavier respectively, and $1.1\times$ and
%$20.0\times$ for the $5 \times 5$ filter on 2080Ti and Xavier respectively. \RV{PyTorch delivers an average speedup of $1.6\times$ and
%$30.1\times$ for the $3 \times 3$ filter on 2080Ti and Xavier respectively, and $1.9\times$ and $19.1\times$ for the $5 \times 5$ filter on
%2080Ti and Xavier respectively. Similarly, TensorFlow achieves an average speedup of $1.8\times$ and $34.6\times$ for the $3 \times 3$
%filter on 2080Ti and Xavier respectively, and $2.2\times$ and $25.3\times$ for the $5 \times 5$ filter on 2080Ti and Xavier respectively.
%Both implementations aim to maximize the GPU utilization but ignore the memory pressure.} PRECOMP and GEMM algorithms need extra memory
%operations to compute output elements. As we have discussed in Section \ref{sec:intro}, depthwise convolution is more sensitive to memory
%performance than pointwise and multi-channel 2D convolutions. Consequently, these algorithms are not suitable for depthwise convolutions.
%Overall, our approach improves cuDNN IMPLICIT by $2.0\times$ and $1.4\times$ when using a $3 \times 3$  filter on 2080Ti and Xavier
%respectively, and $3.5\times$ and $2.1\times$ when using a $5 \times 5$ filter on 2080Ti and Xavier respectively.
%>>>>>>> 1f65a22d2c09b015caafdad443dc01e12c086b41
...
convolution) and can not fully utilize DP4A instruction to accelerate INT8 convolution. \RV{We note that PyTorch and TensorFlow do not
optimize depthwise convolutions on INT8. Nonetheless, our approach gives over $10\times$ speedups when using INT8 over cuDNN, PyTorch and
TensorFlow.}
...
\RV{From Fig. \ref{fig:dwspeedup} we can observe that speedups of our approach over cuDNN IMPLICIT fluctuate in a small range as batch size
increases. The reason is that depthwise convolutions are memory bound and our column and row reuse techniques focus on optimizing memory
performance of depthwise convolutions, thus high GPU utilization contributes to neither our approach nor cuDNN IMPLICIT. }
...
\subsubsection{Setup} In this experiment, \RV{PyTorch and TensorFlow use the cuDNN implementations as their backend. Therefore, we only} compare our approach against all available pointwise convolution implementations in cuDNN.
...
%<<<<<<< HEAD
%When using data type FP32, cuDNN IMPLICIT performs best {\color{red}in 152 out of 180 test cases on 2080Ti and 147 out of 180 test cases on Xavier}. Therefore, we report the results of
%cuDNN IMPLICIT and take cuDNN GEMM as the baseline implementation. When using data type INT8, cuDNN PRECOMP performs best {\color{red}in 180 out of 180 test cases on 2080Ti and 127 out of 180 test cases on Xavier. Hence, we show
%the results of cuDNN PRECOMP. In cuDNN, only PRECOMP and IMPLICIT are supported for INT8, thus we choose cuDNN IMPLICIT as the baseline}. Table \ref{tab:pwconvandparas} lists the layer configurations {\color{red}and parameter values generated for $Warp_H$, $Warp_W$, $Block_{num}$ and $C_{num}$.} The notations can be found in Section \ref{sec:roadmap}.
%
%{\color{red}Normally, for a given convolution layer configuration, when the width of the logical layout of output (Fig. \ref{fig:pwworkflow}) is small, our scheme tends to choose a small $C_{num}$, which can generate more warps to saturate GPU. Otherwise, our scheme tends to choose a large $C_{num}$ to reduce the number of warps because there are already enough warps to saturate GPU. An exceptional parameter tuple (we take parameter tuples generated for 2080Ti as examples) is the layer configuration CONV9 with $I_N = 1$. The width of the logical layout of CONV9 is small, hence we would like to search for a small $C_{num}$. However, in this case, $C_{num} = 32$ is large. The reason is that our scheme finds that different values of $C_{num}$ produce similar GPU utilization, then it tries to maximize AI (Formula \ref{fo:diff}) and then choose $C_{num} = 32$ (the relationship of AI and $C_{num}$ is detailed in Section \ref{sec:cnum}).}
%=======
\RV{To aid clarify, we compare our approach to the best-performing alternative scheme - cuDNN IMPLICIT for FP32 and PRECOMP for INT8. When
using data type INT8, cuDNN PRECOMP performs better than IMPLICIT in 180 out of 180 test cases on 2080Ti and 127 out of 180 test cases on
Xavier. For FP32, we normalize the speedup over cuDNN GEMM. For INT8, because cuDNN GEMM does not support this data type, we show the
speedup over cuDNN IMPLICIT.} Table \ref{tab:pwconvandparas} lists the layer configurations \RV{and parameter values generated for
...
Fig. \ref{fig:pwinfo} reports the measured SM utilizations and ratios of executed LDG instruction counts of cuDNN IMPLICIT to our approach when using a batch size of 32 on 2080Ti. Other configurations have a similar performance.
...
%<<<<<<< HEAD
%{\color{red}From Fig. \ref{fig:pwspeedupfp32} and \ref{fig:pwspeedupint8} we can observe that as batch size increases, speedups of our approach over cuDNN IMPLICIT and cuDNN PRECOMP tend to become lower. The reason is that pointwise convolution is more sensitive to GPU utilization. As batch size increases, generated warps can saturate GPU, thus cuDNN IMPLICIT and PRECOMP can fully utilize GPU and our dynamic tile size scheme will lose its advantage.}
%=======
...
is because that pointwise convolution is more sensitive to GPU utilization. A larger batch size tends to use more warps, which alone can
improve the GPU utilization. By contrast, when the batch size is smaller, the resulting warps alone is insufficient in utilizing the GPU
