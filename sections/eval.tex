...
\begin{itemize}
  ...
  \item N\textbf{VIDIA Performance Primitives (NPP)}. This is an image and signal processing library. We use NPP for 2D convolutions
      only.
  \item \textbf{GEMM-im2col (im2col)}. We extract the implementation of the GEMM-im2col from Caffe \cite{jia2014caffe} and take it as a
...
\begin{figure*}
\centering
\subfloat[Speedups for the filter of size $3 \times 3$.]{\includegraphics[width=0.85\columnwidth,height=4.5cm]{./figure/2d_conv_f3.eps}
	\label{fig:2druntimef3c12080}}
\hspace{0em}
\subfloat[Speedups for the filter of size $5 \times 5$.]{\includegraphics[width=0.85\columnwidth,height=4.5cm]{./figure/2d_conv_f5.eps}
...
\subfloat{\includegraphics[width=0.85\columnwidth,height=4.5cm]{./figure/2dmemthroughput.eps}
	\label{fig:2dmemthr}} \hspace{0em}

\subfloat{\includegraphics[width=0.85\columnwidth,height=4.5cm]{./figure/2dmembandwidth.eps}
	\label{fig:2dmaxband}}
	
\caption{Memory throughput (up) and max bandwidth (down) of NPP and our implementation for the filter of size $3 \times 3$.}
...
\subfloat{\includegraphics[width=0.9\columnwidth,height=5.5cm]{./figure/depthwise_f3.eps}
	\label{fig:f33druntime2080}}
\subfloat{\includegraphics[width=0.9\columnwidth,height=5.5cm]{./figure/depthwise_f5.eps}
	\label{fig:f53druntime2080}}

\caption{Speedups of our approach and cuDNN algorithms over the baseline implementation of the depth-wise convolution. Left is for the $3\times 3$ filter and right is for the $5\times 5$ filter.}
\label{fig:3druntime}
\end{figure*}
...
\subfloat{\includegraphics[width=0.85\columnwidth,height=4.5cm]{./figure/depwisememthroughput.eps}
	\label{fig:depwisememthr}}
\hspace{0em}
\subfloat{\includegraphics[width=0.85\columnwidth,height=4.5cm]{./figure/depwisemembandwidth.eps}
	\label{fig:depwisemaxband}}
	
\caption{Memory throughput (up) and max bandwidth (down) of cuDNN implicit algorithm and our implementation for the filter of size $3 \times 3$.}
...
implementation achieves average speedups of 19.5$\times$ and 25.6$\times$ relative to GEMM-im2col for one and three input channels, respectively. Compared with the fastest algorithm in cuDNN, the proposed approach achieves average speedups of 1.23$\times$ and 1.13$\times$ for one and three input channels, respectively.
...
\subsubsection{Further analysis} Our implementation has a linear scale with channels, whereas cuDNN is highly optimized on channels and exhibits well performance as the number of channels increases.

\subsubsection{Summary} Our reuse algorithms can accelerate multi-channel 2D convolutions with small input channels.
