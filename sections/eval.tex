...
this work, we test 2D, multi-channel 2D and depth-wise convolutions with two filters sizes, $3 \times 3$ and $5 \times 5$, because these
...
\begin{figure*}
\centering
\subfloat[][Speedups when using $3 \times 3$ filter]{\includegraphics[width=0.85\columnwidth]{./figure/2d_conv_f3.eps}
	\label{fig:2druntimef3c12080}}
\hspace{0em}
\subfloat[][Speedups when using a $3 \times 3$ filter]{\includegraphics[width=0.85\columnwidth]{./figure/2d_conv_f5.eps}
...
\subfloat[][$3\times 3$ filter]{\includegraphics[width=0.9\columnwidth]{./figure/depthwise_f3.eps}
	\label{fig:f33druntime2080}}
\subfloat[][$5\times 5$ filter]{\includegraphics[width=0.9\columnwidth]{./figure/depthwise_f5.eps}
...
\subfloat[][Memory throughput]{\includegraphics[width=0.85\columnwidth]{./figure/depwisememthroughput.eps}
	\label{fig:depwisememthr}} \hspace{0em}


\subfloat[][Maximum bandwidth]{\includegraphics [width=0.85\columnwidth]{./figure/depwisemembandwidth.eps}
...
\subsubsection{Setup} In this experiment, we compare our approach against the multi-channel 2D convolution implementations in cuDNN and use GEMM-im2col as the baseline. Since our work focuses on optimizing memory transactions of convolutions but not operations on input channels, we apply our approach to convolutions with one and three input channels, which are typically used in the first few layers of a CNN. {\color{red}Notice that, when applied to a single-channel input, depth-wise convolution uses exactly one filter whereas multi-channel 2D convolution can use one or more filters}. We use the layer configurations four popular CNN models,
...
configurations that have a large number of channels. This can be improved by {\color{red}careful optimizations on input channels}. Nonetheless, our approach significantly boosts the performance for convolution layers with a small number of channels (which is the
