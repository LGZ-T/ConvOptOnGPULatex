...
our approach gives modest slowdown over cuDNN gemm and implicit. {\color{red}Layer CONV20 uses 1024 filters to convolve the input and both cuDNN and our approaches can generate enough thread blocks to saturate GPU, which makes memory performance more prominent. The assembly level optimizations in cuDNN can hide global memory access latency well (as shown in Fig. \ref{fig:stalllongscore}), and thus achieves better performance than our approach.} On average, our implementation and cuDNN implicit achieve an average speedup of $3\times$ and
...
optimizing pointwise convolution because {\color{red}utilizing more SMs can significantly accelerate the computation}.	As can be seen from Fig. \ref{fig:pwsmutil}, our approach has an average of
...
dynamic block size scheme (Section \ref{sec:pwinputdependent}) overcome this limitation by adaptively determining the right block size to use at
