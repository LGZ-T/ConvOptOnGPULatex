...
\begin{figure}[t!]
    ...
    \qquad
    %\vspace{5mm}
    \subfloat[][The ratio of executed LDG (load from global memory) instruction counts given by the cuDNN implicit algorithm over our approach.]{\includegraphics[width=0.9\columnwidth]{./figure/ldginst.eps}\label{fig:dwldginst}}
    \vspace{-2mm}
...
convolutions on two platforms. The baseline is cuDNN gemm. Our approach achieves the best performance for all layers except for CV20 where
our approach gives modest slowdown over cuDNN gemm and implicit. Layer CV20 uses 1024 filters to convolve the input, \FIXME{explain why our
approach gives slowdown performance.}. On average, our implementation and cuDNN implicit achieve an average speedup of $3\times$ and
...
optimizing pointwise convolution because \FIXME{xxx}.	As can be seen from Fig. \ref{fig:pwsmutil}, our approach has an average of
...
dynamic block size scheme (\FIXME{Section \ref{}}) overcome this limitation by adaptively determining the right block size to use at
