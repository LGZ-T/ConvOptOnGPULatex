...
found IMPLICIT gives the best performance {\color{red} in all test cases. Therefore, we report the results by comparing our approach against the IMPLICIT algorithm of
...
$3.9\times$ and $39.4\times$ for the $5 \times 5$ filter on 2080Ti and Xavier respectively. The cuDNN IMPLICIT algorithm achieves an average
speedup of $1.1\times$ and $32.8\times$ for the $3 \times 3$ filter on 2080Ti and Xavier respectively, and $1.1\times$ and $20.0\times$ for
the $5 \times 5$ filter on 2080Ti and Xavier respectively. 
{\color{red}PyTorch achieves an average speedup of $1.6\times$ and $30.1\times$ for the $3 \times 3$ filter on 2080Ti and Xavier respectively, and $1.9\times$ and $19.1\times$ for
the $5 \times 5$ filter on 2080Ti and Xavier respectively. TensorFlow achieves an average speedup of $1.8\times$ and $34.6\times$ for the $3 \times 3$ filter on 2080Ti and Xavier respectively, and $2.2\times$ and $25.3\times$ for
the $5 \times 5$ filter on 2080Ti and Xavier respectively.
PyTorch and TensorFlow implementations focus on increasing GPU utilization without considering memory performance.} PRECOMP and GEMM
...
\subsubsection{Setup} In this experiment, {\color{red}PyTorch and TensorFlow use cuDNN implementations. Therefore we only need to} compare our approach against all available pointwise convolution implementations in cuDNN.
...
the results of cuDNN PRECOMP. In cuDNN, only PRECOMP and IMPLICIT are supported for INT8, thus we choose cuDNN IMPLICIT as the baseline}. Table \ref{tab:pwconvandparas} lists the layer configurations {\color{red}and parameter values generated for $Warp_H$, $Warp_W$, $Block_{num}$ and $C_{num}$.} The notations can be found at Section \ref{sec:roadmap}.

{\color{red}Normally, for a given convolution layer configuration, when the width of the logical layout of output (Fig. \ref{fig:pwworkflow}) is small, our scheme tends to choose a small $C_{num}$, which can generate more warps to saturate GPU. On the other hand, our scheme tends to choose a large $C_{num}$ to reduce the number of warps because there are already enough warps to saturate GPU. An exceptional parameter tuple (we take parameter tuples generated for 2080Ti as examples) is the layer configuration \textbf{CONV9} with $I_N = 1$. The width of the logical layout of \textbf{CONV9} is small, hence we would like to search for a small $C_{num}$. However, in this case, $C_{num} = 32$ is large. The reason is that our scheme finds that different values of $C_{num}$ produce similar GPU utilization, then it tries to maximize AI (Formula \ref{fo:diff}) and then choose $C_{num} = 32$ (the relationship of AI and $C_{num}$ is detailed in Section \ref{sec:cnum}).}
