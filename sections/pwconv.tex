...
As each thread can use up to 256 registers, a thread block can use at most $4 \times 32 \times 256=32768$ registers, which is only half of the registers of an SM (on RTX 2080Ti, each SM contains 65536 registers).
...
Therefore, when searching for the best combination, we only need to set $Block_{num}=\{2,4\}$.
...
In our design, when $Warp_H > 12$, we need assembly level optimizations like the work in \cite{yan2020optimizing} for some configurations of pointwise convolution to avoid register spills.
