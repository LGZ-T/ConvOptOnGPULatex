...
This section elaborates how to perform column reuse to eliminate duplicate columns. The process of column reuse is presented in Figure \ref{fig:corealgo}.%Each thread loads the first 5 corresponding input elements (each thread needs to load $F_H \times F_W \times F_C = 5 \times 5 \times 1 = 25$ input elements in total) to calculate the convolution of one output element.
...
The input elements 1, 2 and 3 loaded in step 2 are already loaded by threads $t1$, $t2$ and $t3$ in
