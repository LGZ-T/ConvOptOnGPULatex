...
\ref{sec:rowreuse}), for reducing the number of GPU memory accesses for depthwise convolution. {\color{red}Both optimizations are proposed and briefly described in our previous work \cite{lu2020optimizing}. In this paper, we give a detailed description of both optimizations.}
...
different threads. {\color{red}This optimized implementation is introduced in our previous work \cite{lu2020optimizing}}. Fig. \ref{fig:optalgo1} depicts such an optimization. Specifically, in steps 1 and 2 of Fig. \ref{fig:optalgo1},
...
Pseudo codes at Lines 1-5 process the first $F_H-1$ rows ($row_{i0}$ and $row_{i1}$ in Fig. \ref{algo:rowreuse}) that are needed by less than $F_H$ output elements.
Codes at lines 6-11 process the rows needed by exact $F_H$ output elements (e.g., $row_{i2}$ in Fig. \ref{algo:rowreuse}).
Finally, codes at Lines 12-17 process the last $F_H-1$ rows, which are needed by less than $F_H$ output elements (e.g., $row_{i3}$ and $row_{i4}$ in Fig. \ref{algo:rowreuse}).
