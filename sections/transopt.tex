...
%In Section \ref{sec:together}, we show how the two optimization techniques can be used together.
%Then we describe how to distribute input channels among threads within a warp and how many threads to process one output element for pointwise convolution (Section {\color{red}}).
...
\mypara{Working example.} We use the depthwise convolution with only one channel shown in Figure \ref{fig:twostrategies} as a working example to explain our
column reuse method. 
...
\subsubsection{Standard convolution} Figure \ref{fig:directalgo} shows a standard depthwise convolution operation, operating on a single-channel input. Here, each thread loads the first corresponding input elements from the GPU global memory. Given that the
...
\mypara{Working example.} Consider the standard convolution example shown in Figure \ref{fig:rowreuse} as a working example for our row
...
\subsubsection{Standard convolution} Assume we use one thread to calculate one column of output elements. For the working example given in Figure
