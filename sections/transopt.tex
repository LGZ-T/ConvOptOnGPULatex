...
%{\color{red}Both optimizations are proposed and briefly described in our previous work \cite{lu2020optimizing}. In this paper, we give a
%detailed description of both optimizations.}
...
redundant memory accesses and unnecessary memory access latency. Even if the elements may be prefetched to the L1 cache before the next step,
...
data are stored in the local memory). \RV{Note that this approach will not improve the register usage.} Since steps 4 and 5 of our implementation (Fig. \ref{fig:optalgo2}) adopt a similar procedure as
...
%{\color{red}Delete:For example, and as illustrated in Fig. \ref{fig:split7x7}, a $7 \times 7$ filter can be divided into a $7 \times 5$ sub-filter and a $7 \times 3$ sub-filter with one overlap column.}
...
%For our working example, the computation of element $out_0$ in the output column depends on $row_{i0}$. Similarly, output elements $out_0$ and $out_1$ will need $row_{i1}$. 
